...
***********************************************************************************/
	...
	$codigo = $_GET["codigo"];
        ...
        $parametrosIndex = "fltnumprotocolo=$fltnumprotocolo&fltsituacao=$fltsituacao"; //parametros a ser passado para a pagina de detalhamento, fazendo com que ao voltar para o index traga as informaчѕes passadas anteriormente
        //-----
	...
	$erro="";
        ...
        if ($_POST['acao'])
        {
            ...
            if ($acao == "Enviar")
            {
                    ...
                    //recupera o proximo tipo de solicitaчуo, caso retorne falso, deu erro
                    if(Solicitacao::getProximoTipoSolicitacao($idsolicitacao,$idtiposolicitacaorecurso,$erro))
                    {   
                        //se nao existir solicitaчуo original
                        if (empty($idsolicitacaoorigem))
                            $sol->setIdSolicitacaoOrigem($idsolicitacao); //o recurso terс a solicitaчуo atual como original
                        else
                            $sol->setIdSolicitacaoOrigem($idsolicitacaoorigem); //o recurso manterс a solicitaчуo original
